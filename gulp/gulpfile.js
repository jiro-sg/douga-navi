...
// 入出力先指定
const srcBase = "../src";
const themeName = "douga-navi"; // テーマフォルダ名と合わせる
...
const browserSyncOption = {
  proxy: "http://douga-navi.local", // LocalのSite hostを入れる
};
...
const imgImagemin = () => {
  return src(srcPath.img)
    ...
    .pipe(dest(distPath.img)) // 圧縮した画像を保存
    .pipe(webp()) // WebP形式に変換
    .pipe(dest(distPath.img)); // WebP画像を保存
};
